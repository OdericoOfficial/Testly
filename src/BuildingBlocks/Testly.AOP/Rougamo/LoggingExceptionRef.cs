...
namespace Testly.AOP.Rougamo
{
    ...
    public class LoggingExceptionRefAttribute<TRet> : LoggingExceptionAttribute
        where TRet : class, new()
    {
        protected override void HandledException(MethodContext context)
            => context.HandledException(this, new TRet());
    }
}
...
namespace Testly.AOP.Rougamo
{
    public struct LoggingExceptionRef<TRet> : IMo
        where TRet : class, new()
    {
